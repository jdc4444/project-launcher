...
async function startProject(project) {
  ...
  if (processes.has(name)) {
    ...
    if (existing.status === 'running') {
      return { ok: true, port: existing.port, status: 'running', adopted: true };
    }
  }
  ...
  if (port) {
    ...
    if (pids.length > 0) {
      ...
      if (alive) {
        ...
        broadcast('status', { name, status: 'running', port, pid: pids[0] });
        return { ok: true, port, pid: pids[0], status: 'running', adopted: true };
      }
...
async function detectRunning(projects) {
  ...
  for (const p of projects) {
    ...
    if (pids.length > 0) {
      const alive = await healthCheck(p.port);
      if (alive) {
        ...
        broadcast('status', { name, status: 'running', port: p.port, pid: pids[0] });
        found++;
      }
    }
  }
  ...
  for (const p of pollProjects) {
    ...
    if (alive && (!known || known.status !== 'running')) {
      ...
      processes.set(name, {
        ...
        startedAt: new Date().toISOString(),
        status: 'running', logs: [], external: true,
      });
      broadcast('status', { name, status: 'running', port: p.port });
