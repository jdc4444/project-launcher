...
function generateMockHTML(projectDir, projectName) {
  ...
  for (const pyFile of pyFiles) {
    ...
    if (/import\s+plotly|from\s+plotly/.test(source)) {
      return renderPlotlyHTML(source, projectName);
    }
  }
...
// Generate mock Plotly visualization
function renderPlotlyHTML(source, appName) {
  const title = source.match(/title\s*=\s*["'](.+?)["']/)?.[1] || appName;
  const has3D = /Scatter3d|Surface|go\.Mesh3d/.test(source);
  const hasMap = /Scattergeo|Choropleth|scattermapbox/.test(source);

  const vizType = has3D ? '3d' : hasMap ? 'map' : 'chart';

  const vizHTML = vizType === '3d' ? `
    <div style="width:100%;height:500px;background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);border-radius:8px;position:relative;overflow:hidden">
      <svg viewBox="0 0 400 300" style="width:100%;height:100%">
        <defs><linearGradient id="g1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style="stop-color:#4ecdc4;stop-opacity:0.8"/><stop offset="100%" style="stop-color:#556270;stop-opacity:0.4"/></linearGradient></defs>
        <!-- Globe wireframe -->
        <ellipse cx="200" cy="150" rx="120" ry="120" fill="none" stroke="#4ecdc4" stroke-width="0.5" opacity="0.3"/>
        <ellipse cx="200" cy="150" rx="120" ry="40" fill="none" stroke="#4ecdc4" stroke-width="0.5" opacity="0.3"/>
        <ellipse cx="200" cy="150" rx="40" ry="120" fill="none" stroke="#4ecdc4" stroke-width="0.5" opacity="0.3"/>
        <ellipse cx="200" cy="150" rx="80" ry="120" fill="none" stroke="#4ecdc4" stroke-width="0.5" opacity="0.3"/>
        <ellipse cx="200" cy="150" rx="120" ry="80" fill="none" stroke="#4ecdc4" stroke-width="0.5" opacity="0.3"/>
        <!-- Data points -->
        <circle cx="160" cy="110" r="4" fill="#ff6b6b"/><circle cx="220" cy="130" r="4" fill="#ff6b6b"/>
        <circle cx="180" cy="170" r="4" fill="#ff6b6b"/><circle cx="250" cy="100" r="4" fill="#ff6b6b"/>
        <circle cx="140" cy="160" r="4" fill="#ff6b6b"/><circle cx="230" cy="180" r="4" fill="#ff6b6b"/>
        <text x="160" y="100" fill="#fff" font-size="8" text-anchor="middle">City 1</text>
        <text x="220" y="120" fill="#fff" font-size="8" text-anchor="middle">City 2</text>
      </svg>
      <div style="position:absolute;bottom:12px;right:12px;background:rgba(0,0,0,0.5);color:#aaa;padding:4px 8px;border-radius:4px;font-size:11px">plotly</div>
    </div>` : `
    <div style="width:100%;height:400px;background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:20px;position:relative">
      <svg viewBox="0 0 400 250" style="width:100%;height:100%">
        <line x1="40" y1="220" x2="380" y2="220" stroke="#e2e8f0" stroke-width="1"/>
        <line x1="40" y1="20" x2="40" y2="220" stroke="#e2e8f0" stroke-width="1"/>
        <polyline points="60,180 100,120 140,150 180,80 220,100 260,60 300,90 340,40" fill="none" stroke="#636efa" stroke-width="2"/>
        <polyline points="60,190 100,170 140,180 180,140 220,160 260,120 300,140 340,100" fill="none" stroke="#ef553b" stroke-width="2"/>
      </svg>
    </div>`;
...
<html><head><meta charset="UTF-8"><style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Open Sans', -apple-system, sans-serif; background: #fff; }
