...
function normalizeSyntheticCollections(list) {
  if (!Array.isArray(list)) return [];
  return list.map(item => {
    const name = typeof item?.name === 'string' ? item.name.trim() : '';
    const targetName = typeof item?.targetName === 'string' ? item.targetName.trim() : name;
    const members = Array.isArray(item?.members)
      ? item.members.filter(m => typeof m === 'string').map(m => m.trim()).filter(Boolean)
      : [];
    const deduped = Array.from(new Set([targetName, ...members].filter(Boolean)));
    if (!name || deduped.length === 0) return null;
    return { name, targetName, members: deduped };
  }).filter(Boolean);
}

function readOverridesConfig() {
  const raw = readJSON(OVERRIDES_FILE) || {};
  if (raw.projects || raw.syntheticCollections) {
    return {
      projects: raw.projects || {},
      syntheticCollections: normalizeSyntheticCollections(raw.syntheticCollections),
    };
  }
  return {
    projects: raw,
...
function writeOverridesConfig(config) {
  const payload = {
    projects: config?.projects || {},
    syntheticCollections: normalizeSyntheticCollections(config?.syntheticCollections),
  };
  const hasProjects = Object.keys(payload.projects).length > 0;
  const hasSyntheticCollections = payload.syntheticCollections.length > 0;
  const output = hasProjects || hasSyntheticCollections ? payload : {};
...
// Fields: port, category, entrypoint, thumbnail, startCommand, startArgs, framework, protocol
...
function scanDir(projectDir, name, launchMap, overrides) {
  ...
  if (plrc) {
    ...
    if (plrc.protocol) detected.protocol = plrc.protocol;
  ...
  if (launchInfo) {
    // Keep command/args sourced together so we don't build invalid hybrids
    // like `npm /abs/path/server.js` from auto-detection + launch.json.
    if (plrc?.startCommand || plrc?.startArgs || plrc?.entrypoint) {
      ...
      detected.startCommand = launchInfo.runtimeExecutable || detected.startCommand;
    ...
    if (!plrc?.protocol && launchInfo.protocol) detected.protocol = launchInfo.protocol;
...
function applySyntheticCollections(projects, syntheticCollections) {
  if (!syntheticCollections.length) return projects;
  ...
  for (const project of projects) {
    if (project.type === 'collection') {
      ...
      for (const child of project.children || []) {
        projectLookup.set(getProjectKey(child), child);
      }
      continue;
    }
  ...
  for (const spec of syntheticCollections) {
    const targetName = spec.targetName || spec.name;
    const memberNames = Array.from(new Set([targetName, ...(spec.members || [])]));
    const members = memberNames
      .map(memberName => projectLookup.get(memberName))
      .filter(Boolean)
      .filter(project => !groupedKeys.has(getProjectKey(project)));

    if (members.length < 2) continue;

    members.sort((a, b) => {
      if (getProjectKey(a) === targetName) return -1;
      if (getProjectKey(b) === targetName) return 1;
      return a.name.localeCompare(b.name);
    });

    for (const member of members) {
      member.syntheticGroupKey = targetName;
      groupedKeys.add(getProjectKey(member));
    }

    const anchor = members.find(member => getProjectKey(member) === targetName) || members[0];
    synthetic.push({
      name: spec.name,
      dir: anchor.dir,
      type: 'collection',
      ...
      isGit: members.some(member => member.isGit),
      launchable: false,
      capturable: false,
      children: members.map(member => ({ ...member, syntheticParentName: spec.name })),
      childCount: members.length,
      launchableChildren: members.filter(member => member.launchable).length,
      synthetic: true,
      targetName,
    });
  }

  const merged = [...projects, ...synthetic];
  sortProjectList(merged);
  return merged;
}
...
function scanProjects() {
  ...
  const overrides = overridesConfig.projects || {};
  ...
  return applySyntheticCollections(projects, overridesConfig.syntheticCollections || []);
}
...
function syncLaunchJSON(projects) {
  ...
  for (const p of allProjects) {
    ...
    const existingByCwd = byCwd.get(p.dir);
    const existingByName = byName.get(p.launchName || p.name);
    const existing = existingByCwd || existingByName;
    if (existing) {
      let updatedExisting = false;
      const desiredName = p.fullName || p.name;
      const desiredPort = p.port || existing.port;
      const oldCwd = existing.cwd;

      // If a project moved, repair the stale launch.json path instead of
      // preserving it forever via the name fallback.
      if (oldCwd !== p.dir) {
        existing.cwd = p.dir;
        updatedExisting = true;
        if (oldCwd) byCwd.delete(oldCwd);
        byCwd.set(p.dir, existing);
        console.log(`[registry] Repointed ${desiredName} from ${oldCwd || '(none)'} to ${p.dir}`);
      }

      // Keep the registry name aligned with the scanned project name/fullName.
      if (existing.name !== desiredName) {
        existing.name = desiredName;
        updatedExisting = true;
        byName.set(desiredName, existing);
      }

      // .plrc/detected ports should be able to correct stale registry ports.
      if (desiredPort && existing.port !== desiredPort) {
        if (existing.port) usedPorts.delete(existing.port);
        existing.port = desiredPort;
        usedPorts.add(desiredPort);
        updatedExisting = true;
      }

      if ((existing.protocol || null) !== (p.protocol || null)) {
        if (p.protocol) existing.protocol = p.protocol;
        else delete existing.protocol;
        updatedExisting = true;
      }

      p.port = existing.port || p.port;
    ...
    if (p.protocol) config.protocol = p.protocol;
