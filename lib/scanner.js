...
function scanDir(projectDir, name, launchMap, overrides) {
  ...
  // Categorize: utility = no web UI (scripts, MCP plugins, data tools, skills)
  // A project is a utility if it's not launchable, OR if it's launchable but has no port (no web UI)
  let category = 'app';
  if (!detected.nativeApp && (!launchable || (!detected.port && detected.type !== 'html'))) {
    category = 'utility';
  }
