...
function setScreenshotFromFile(screenshotName, sourceImagePath, sha) {
  ...
  // Generate thumbnail using sharp if available, else just copy
  try {
    ...
    sharp(sourceImagePath)
      ...
      .png()
      .toFile(thumbPath)
      .catch(() => fs.copyFileSync(sourceImagePath, thumbPath));
  } catch {
    fs.copyFileSync(sourceImagePath, thumbPath);
  }

  return { full: `${sha}.png`, thumb: `${sha}-thumb.png` };
}

module.exports = { captureScreenshot, getScreenshots, getLatestScreenshot, getScreenshotPath, importFallbackImage, findFallbackImage, listCandidateImages, setScreenshotFromFile, SCREENSHOTS_DIR };
