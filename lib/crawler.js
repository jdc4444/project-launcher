  ...
  try {
    ...
    if (project.port) {
      const alreadyUp = await new Promise(resolve => {
        const req = http.get(`http://localhost:${project.port}`, { timeout: 2000 }, () => resolve(true));
        req.on('error', () => resolve(false));
        req.on('timeout', () => { req.destroy(); resolve(false); });
      });
      if (alreadyUp) {
        console.log(`[crawler] Screenshotting ${name} on existing port ${project.port}...`);
        await withTimeout(captureScreenshot(name, project.port, sha), 60000, `screenshot ${name}`);
